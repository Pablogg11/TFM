...
class Experiment:
    ...
    def generate_explanations(self, trained_model, explainer):
        ...
        if (explainer.name == "breakdown" or explainer.name == "ibreakdown"):
            explainer = explainer.explainer(trained_model, self.dataset.data)
        else:
            explainer = explainer.explainer(trained_model, X_train) if explainer.name == "axom" else explainer.explainer(trained_model.predict, X_train)
